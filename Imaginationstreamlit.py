...
opback = st.selectbox('Select Background OPTIONAL',(FALSE, TRUE),)
...
#Logo de marca
oplogo = st.selectbox('Do you want to include brand logo?',(TRUE ,FALSE))
st.write('You selected:', oplogo)
...
if resultado == True:
    ...
    titulo_alto = bbox[3] - bbox[1]

    claim = opclaim
    #claim_ancho, claim_alto = font_claim.getsize(claim)
    bbox = draw.textbbox((0, 0), claim, font=font_claim)
    claim_ancho = bbox[2] - bbox[0]
    claim_alto = bbox[3] - bbox[1]

    cta = opctatext
    #cta_ancho, cta_alto = font_claim.getsize(cta)
    bbox = draw.textbbox((0, 0), cta, font=font_claim)
    cta_ancho = bbox[2] - bbox[0]
    cta_alto = bbox[3] - bbox[1]
    ...
    if tipo == "C":
        ...
        if oplogo == TRUE:
            ...
            composite_image = Image.alpha_composite(composite_image, img_logo)
        ...
        if titulo_ancho >= ancho*0.95:
            ...
            for k in range(100):
                ...
                bbox = draw.textbbox((0, 0), titulo[0:split_point] , font=font_title)
                titulo_ancho = bbox[2] - bbox[0]
                titulo_alto = bbox[3] - bbox[1]
    ...
    #Agrego Claim
        ...
        if claim_ancho >= ancho*0.95:
            ...
            for k in range(100):
                ...
                bbox = draw.textbbox((0, 0), claim[0:split_point] , font=font_claim)
                claim_ancho = bbox[2] - bbox[0]
                claim_alto = bbox[3] - bbox[1]
    ...
    #Agrego Call to action
        if callToAction == True:
            ...
            bbox = draw.textbbox((0, 0), text , font=font_claim)
    ...
    if tipo == "H":
        ...
        if productos ==2:
            img_prod10 = Image.new("RGBA", (ancho, alto), transparent_color)
            im_prod11 = Image.open("./Productos/190679000026.png")
            im_prod12 = im_prod11.resize((int(min(ancho, alto)*0.7*multiplicador2),int(min(ancho, alto)*0.7*multiplicador2)))
            x1, y1 = find_first_non_transparent_pixel(im_prod12)
            xf1, yf1 = find_last_non_transparent_pixel(im_prod12)
            crop_im_prod12 = im_prod12.crop((x1, 0, xf1 + 1, im_prod12.height ))
